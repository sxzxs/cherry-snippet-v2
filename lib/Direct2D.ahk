...
class ID2D1Factory extends ID2DBase {
	...
	__New(p := 0) {
		#DllLoad 'd2d1.dll'
		if (!p) {
			if (CLSIDFromString('{06152247-6f50-465a-9245-118bfd3b6007}', buf := Buffer(16, 0)))
				throw OSError(A_LastError)
			DllCall('d2d1\D2D1CreateFactory', 'uint', 0, 'ptr', buf, 'uint*', 0, 'ptr*', &pIFactory := 0)
			if (!pIFactory)
				throw OSError(A_LastError)
			this.ptr := pIFactory
		} else this.ptr := p
...
class ID2D1RenderTarget extends ID2D1Resource {
	...
	CreateBitmap(size, srcData, pitch, bitmapProperties) {	; D2D1_SIZE_U , D2D1_BITMAP_PROPERTIES
		ComCall(4, this, 'ptr', NumGet(size, 'ptr'), 'ptr', srcData, 'uint', pitch, 'ptr', bitmapProperties, 'ptr*', &bitmap := 0)
	...
	*/
	...
	DrawLine(point0, point1, brush, strokeWidth, strokeStyle) {	; D2D1_POINT_2F, D2D1_POINT_2F, ID2D1Brush, ID2D1StrokeStyle
		ComCall(15, this, 'ptr', NumGet(point0, 'ptr'), 'ptr', NumGet(point1, 'ptr'), 'ptr', brush, 'float', strokeWidth, 'ptr', strokeStyle, 'int')
	}
	...
	DrawTextLayout(origin, textLayout, defaultForegroundBrush, options := 0) {	; D2D1_POINT_2F , IDWriteTextLayout , ID2D1Brush , D2D1_DRAW_TEXT_OPTIONS
		ComCall(28, this, 'ptr', NumGet(origin, 'ptr'), 'ptr', textLayout, 'ptr', defaultForegroundBrush, 'uint', options, 'int')
	}
	...
	DrawGlyphRun(baselineOrigin, glyphRun, foregroundBrush, measuringMode := 0) {	; D2D1_POINT_2F , DWRITE_GLYPH_RUN , ID2D1Brush , DWRITE_MEASURING_MODE
		ComCall(29, this, 'ptr', NumGet(baselineOrigin, 'ptr'), 'ptr', glyphRun, 'ptr', foregroundBrush, 'int', measuringMode, 'int')
	}
	...
	*/
	...
	GetPixelFormat() {
		ComCall(50, this, 'ptr', format := direct2d_struct('D2D1_PIXEL_FORMAT'), 'int')
		return format
	}
...
class ID2D1Brush extends ID2D1Resource {
	...
	GetTransform() {
		ComCall(7, this, 'ptr', transform := direct2d_struct('D2D1_MATRIX_3X2_F'), 'int')
		return transform
	}
}
...
class ID2D1SolidColorBrush extends ID2D1Brush {
	...
	GetColor() {
		ComCall(9, this, 'ptr', color := direct2d_struct('D2D1_COLOR_F'), 'int')
		return color
	}
}
class ID2D1LinearGradientBrush extends ID2D1Brush {
	...
	GetStartPoint() {
		ComCall(10, this, 'ptr', point := direct2d_struct('D2D1_POINT_2F'), 'int')
		return point
	}

	; Retrieves the ending coordinates of the linear gradient.
	GetEndPoint() {
		ComCall(11, this, 'ptr', point := direct2d_struct('D2D1_POINT_2F'), 'int')
		return point
	}
...
class ID2D1RadialGradientBrush extends ID2D1Brush {
	...
	GetCenter() {
		ComCall(12, this, 'ptr', point := direct2d_struct('D2D1_POINT_2F'), 'int')
		return point
	}

	; Retrieves the offset of the gradient origin relative to the gradient ellipse's center.
	GetGradientOriginOffset() {
		ComCall(13, this, 'ptr', point := direct2d_struct('D2D1_POINT_2F'), 'int')
		return point
	}
...
class ID2D1Geometry extends ID2D1Resource {
	...
	GetBounds(worldTransform := 0) {	; D2D1_MATRIX_3X2_F
		bounds := direct2d_struct('D2D1_RECT_F')
	...
	StrokeContainsPoint(point, strokeWidth, strokeStyle, worldTransform, flatteningTolerance) {	; D2D1_POINT_2F, float, ID2D1StrokeStyle , D2D1_MATRIX_3X2_F, float
		ComCall(6, this, 'ptr', NumGet(point, 'ptr'), 'float', strokeWidth, 'ptr', strokeStyle, 'ptr', worldTransform, 'float', flatteningTolerance, 'int*', &con := 0)
		return con
	}

	; Indicates whether the area filled by the geometry would contain the specified point given the specified flattening tolerance.
	FillContainsPoint(point, worldTransform, flatteningTolerance) {	; D2D1_POINT_2F , D2D1_MATRIX_3X2_F, float
		ComCall(7, this, 'ptr', NumGet(point, 'ptr'), 'ptr', worldTransform, 'float', flatteningTolerance, 'int*', &con := 0)
		return con
	}
	...
	*/
	...
	ComputePointAtLength(length, worldTransform, flatteningTolerance) {	; D2D1_MATRIX_3X2_F
		point := direct2d_struct('D2D1_POINT_2F')
		unitTangentVector := direct2d_struct('D2D1_POINT_2F')
...
class ID2D1RectangleGeometry extends ID2D1Geometry {
	...
	GetRect() {
		rect := direct2d_struct('D2D1_RECT_F')
...
class ID2D1RoundedRectangleGeometry extends ID2D1Geometry {
	...
	GetRoundedRect() {
		roundedRect := direct2d_struct('D2D1_ROUNDED_RECT')
...
class ID2D1EllipseGeometry extends ID2D1Geometry {
	...
	GetEllipse() {
		ellipse := direct2d_struct('D2D1_ELLIPSE')
...
class ID2D1TransformedGeometry extends ID2D1Geometry {
	...
	GetTransform() {
		transform := direct2d_struct('D2D1_MATRIX_3X2_F')
...
class ID2D1GradientStopCollection extends ID2D1Resource {
	...
	GetGradientStops(gradientStopsCount) {
		gradientStops := direct2d_struct('D2D1_GRADIENT_STOP[' gradientStopsCount ']')
...
class ID2D1DrawingStateBlock extends ID2D1Resource {
	...
	GetDescription() {
		stateDescription := direct2d_struct('D2D1_DRAWING_STATE_DESCRIPTION')
...
class ID2D1SimplifiedGeometrySink extends ID2DBase {
	...
	BeginFigure(startPoint, figureBegin) {
		ComCall(5, this, 'ptr', NumGet(startPoint, 'ptr'), 'int', figureBegin, 'int')
	}
...
class IDWriteFactory extends ID2DBase {
	...
	__New(p := 0) {
		#DllLoad 'dwrite.dll'
		if (!p) {
			if (CLSIDFromString('{b859ee5a-d838-4b5b-a2e8-1adc7d93db48}', buf := Buffer(16, 0)))
				throw OSError(A_LastError)
			DllCall('dwrite\DWriteCreateFactory', 'uint', 0, 'ptr', buf, 'ptr*', &pIFactory := 0)
			if (!pIFactory)
				throw OSError(A_LastError)
			this.ptr := pIFactory
		} else this.ptr := p
...
class IDWriteBitmapRenderTarget extends ID2DBase {
	...
	GetCurrentTransform() {
		ComCall(7, this, 'ptr', transform := direct2d_struct('DWRITE_MATRIX'))
		return transform
	}
...
class IDWriteFontFace extends ID2DBase {
	...
	GetMetrics() {
		ComCall(8, this, 'ptr', fontFaceMetrics := direct2d_struct('DWRITE_FONT_METRICS'))
	...
	GetDesignGlyphMetrics(glyphIndices, glyphCount, isSideways := false) {	; ushort[], uint, DWRITE_GLYPH_METRICS[]
		ComCall(10, this, 'ptr', glyphIndices, 'uint', glyphCount, 'ptr', glyphMetrics := direct2d_struct('DWRITE_GLYPH_METRICS[' glyphCount ']'), 'int', isSideways)
		return glyphMetrics
	}
	GetGlyphIndices(codePoints, codePointCount) {	; uint[], uint
		ComCall(11, this, 'ptr', codePoints, 'uint', codePointCount, 'ptr', glyphIndices := Struct('UINT16[' codePointCount ']'))
		return glyphIndices
	}
	...
	GetGdiCompatibleMetrics(emSize, pixelsPerDip, transform) {	; float, float, DWRITE_MATRIX
		ComCall(16, this, 'float', emSize, 'float', pixelsPerDip, 'ptr', transform, 'ptr', fontFaceMetrics := direct2d_struct('DWRITE_FONT_METRICS'))
		return fontFaceMetrics
	}
	GetGdiCompatibleGlyphMetrics(emSize, pixelsPerDip, transform, useGdiNatural, glyphIndices, glyphCount, isSideways := false) {	; float, float, DWRITE_MATRIX, BOOL, UINT16[], UINT, BOOL
		ComCall(17, this, 'float', emSize, 'float', pixelsPerDip, 'ptr', transform, 'int', useGdiNatural, 'ptr', glyphIndices, 'uint', glyphCount, 'ptr', glyphMetrics := direct2d_struct('DWRITE_GLYPH_METRICS'), 'int', isSideways)
		return glyphMetrics
	}
}
...
class IDWriteTextFormat extends ID2DBase {
	...
	GetTrimming(&trimmingOptions, &trimmingSign) {
		ComCall(17, this, 'ptr', trimmingOptions := direct2d_struct('DWRITE_TRIMMING'), 'ptr*', &trimmingSign := 0)
...
class IDWriteTextLayout extends IDWriteTextFormat {
	...
	GetMetrics() {
		ComCall(60, this, 'ptr', textMetrics := direct2d_struct('DWRITE_TEXT_METRICS'))
		return textMetrics
	}
	GetOverhangMetrics() {
		ComCall(61, this, 'ptr', overhangs := direct2d_struct('DWRITE_OVERHANG_METRICS'))
		return overhangs
	}
...
class IDWritePixelSnapping extends ID2DBase {
	...
	GetCurrentTransform(clientDrawingContext) {
		ComCall(4, this, 'ptr', clientDrawingContext, 'ptr', transform := direct2d_struct('DWRITE_MATRIX'))
		return transform
	}
...
class IDWriteInlineObject extends ID2DBase {
	...
	GetMetrics() {
		ComCall(4, this, 'ptr', metrics := direct2d_struct('DWRITE_INLINE_OBJECT_METRICS'))
		return metrics
	}
	GetOverhangMetrics() {
		ComCall(5, this, 'ptr', overhangs := direct2d_struct('DWRITE_OVERHANG_METRICS'))
		return overhangs
	}
	GetBreakConditions(&breakConditionBefore, &breakConditionAfter) {
...
class IDWriteFont extends ID2DBase {
	...
	GetMetrics() {
		ComCall(11, this, 'ptr', &fontMetrics := direct2d_struct('DWRITE_FONT_METRICS'))
		return fontMetrics
	}
...
class IDWriteGdiInterop extends ID2DBase {
	...
	ConvertFontToLOGFONT(font, &isSystemFont) {
		ComCall(4, this, 'ptr', font, 'ptr', logFont := direct2d_struct('LOGFONTW'), 'int*', &isSystemFont := 0)
		return logFont
	}
	ConvertFontFaceToLOGFONT(font) {
		ComCall(5, this, 'ptr', font, 'ptr', logFont := direct2d_struct('LOGFONTW'))
		return logFont
	}
...
class IDWriteGlyphRunAnalysis extends ID2DBase {
	...
	GetAlphaTextureBounds(textureType) {	; DWRITE_TEXTURE_TYPE
		ComCall(3, this, 'int', textureType, 'ptr', textureBounds := direct2d_struct('D2D1_RECT_U'))
...
class IDWriteTypography extends ID2DBase {
	...
	GetFontFeature(fontFeatureIndex) {
		ComCall(5, this, 'uint', fontFeatureIndex, 'ptr', fontFeature := direct2d_struct('DWRITE_FONT_FEATURE'))
		return fontFeature
	}
}
class ID2DBase {
...
direct2d_struct(name, args*) {
	static D2D1_COLOR_F := 'FLOAT r;FLOAT g;FLOAT b;FLOAT a',
		D2D1_POINT_2U := 'UINT x;UINT y',
		D2D1_POINT_2F := 'FLOAT x;FLOAT y',
		D2D1_RECT_F := 'FLOAT left;FLOAT top;FLOAT right;FLOAT bottom',
		D2D1_RECT_U := 'UINT left;UINT top;UINT right;UINT bottom',
		D2D1_SIZE_F := 'FLOAT width;FLOAT height',
		D2D1_SIZE_U := 'UINT width;UINT height',
		D2D1_MATRIX_3X2_F := 'FLOAT _11;FLOAT _12;FLOAT _21;FLOAT _22;FLOAT _31;FLOAT _32',
		D2D1_PIXEL_FORMAT := 'int format;int alphaMode',
		D2D1_BITMAP_PROPERTIES := 'D2D1_PIXEL_FORMAT pixelFormat;FLOAT dpiX;FLOAT dpiY',
		D2D1_GRADIENT_STOP := 'FLOAT position;D2D1_COLOR_F color',
		D2D1_BRUSH_PROPERTIES := 'FLOAT opacity;D2D1_MATRIX_3X2_F transform',
		D2D1_BITMAP_BRUSH_PROPERTIES := 'int extendModeX;int extendModeY;int interpolationMode',
		D2D1_LINEAR_GRADIENT_BRUSH_PROPERTIES := 'D2D1_POINT_2F startPoint;D2D1_POINT_2F endPoint',
		...
		D2D1_BEZIER_SEGMENT := 'D2D1_POINT_2F point1;D2D1_POINT_2F point2;D2D1_POINT_2F point3',
		D2D1_TRIANGLE := 'D2D1_POINT_2F point1;D2D1_POINT_2F point2;D2D1_POINT_2F point3',
		D2D1_ARC_SEGMENT := 'D2D1_POINT_2F point;D2D1_SIZE_F size;FLOAT rotationAngle;int sweepDirection;int arcSize',
		D2D1_QUADRATIC_BEZIER_SEGMENT := 'D2D1_POINT_2F point1;D2D1_POINT_2F point2',
		D2D1_ELLIPSE := 'D2D1_POINT_2F point;FLOAT radiusX;FLOAT radiusY',
		D2D1_ROUNDED_RECT := 'D2D1_RECT_F rect;FLOAT radiusX;FLOAT radiusY',
		D2D1_STROKE_STYLE_PROPERTIES := 'int startCap;int endCap;int dashCap;int lineJoin;FLOAT miterLimit;int dashStyle;FLOAT ashOffset',
		D2D1_LAYER_PARAMETERS := 'D2D1_RECT_F contentBounds;ptr geometricMask;int maskAntialiasMode;D2D1_MATRIX_3X2_F maskTransform;FLOAT opacity;ptr opacityBrush;int layerOptions',
		D2D1_RENDER_TARGET_PROPERTIES := 'int type;int format;int alphaMode;FLOAT dpiX;FLOAT dpiY;int usage;int minLevel',
		D2D1_HWND_RENDER_TARGET_PROPERTIES := 'HWND hwnd;UINT width;UINT height;int presentOptions',
		D2D1_DRAWING_STATE_DESCRIPTION := 'int antialiasMode;int textAntialiasMode;uint64 tag1;uint64 tag2;D2D1_MATRIX_3X2_F transform',
		D2D1_FACTORY_OPTIONS := 'int debugLevel',
		DWRITE_FONT_METRICS := 'UINT16 designUnitsPerEm; UINT16 ascent; UINT16 descent; INT16 lineGap; UINT16 capHeight; UINT16 xHeight; INT16 underlinePosition; UINT16 underlineThickness; INT16 strikethroughPosition; UINT16 strikethroughThickness;',
		DWRITE_TRIMMING := 'int granularity; UINT32 delimiter; UINT32 delimiterCount;',
		DWRITE_TEXT_METRICS := 'FLOAT left; FLOAT top; FLOAT width; FLOAT widthIncludingTrailingWhitespace; FLOAT height; FLOAT layoutWidth; FLOAT layoutHeight; UINT32 maxBidiReorderingDepth; UINT32 lineCount;',
		DWRITE_OVERHANG_METRICS := 'FLOAT left; FLOAT top; FLOAT right; FLOAT bottom;',
		DWRITE_STRIKETHROUGH := 'FLOAT width; FLOAT thickness; FLOAT offset; int readingDirection; int flowDirection; WCHAR* localeName; int measuringMode;',
		DWRITE_INLINE_OBJECT_METRICS := 'FLOAT width; FLOAT height; FLOAT baseline; BOOL supportsSideways;',
		DWRITE_FONT_FEATURE := 'int nameTag; UINT32 parameter;',
		DWRITE_SCRIPT_ANALYSIS := 'UINT16 script; int shapes;',
		LOGFONTW := 'LONG lfHeight; LONG lfWidth; LONG lfEscapement; LONG lfOrientation; LONG lfWeight; BYTE lfItalic; BYTE lfUnderline; BYTE lfStrikeOut; BYTE lfCharSet; BYTE lfOutPrecision; BYTE lfClipPrecision; BYTE lfQuality; BYTE lfPitchAndFamily; WCHAR lfFaceName[32];'
	return Struct(InStr(name, '[') ? name : %name%, args*)
}
